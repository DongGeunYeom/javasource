...
<%--
	...
	해당 객체에 유효한 범위 안에서 필요한 객체(데이터)들을 저장하고 읽어 들임으로써
	서로 공유할 수 있는 특정한 영역을 가지고 있음
	
	1) page : 현재 서비스되고 있는 페이지의 유효 범위를 가짐
	...
	3) session : session 유효 범위를 가짐
				 HttpSession session => 유지시간만큼 유효 범위
				 브라우저 열고 닫기 전까지 유효 범위
				 로그인, 장바구니 => 서버에 저장
				
	4) application : 서버의 시작과 끝의 유효 범위를 가짐
 --%>
<%
